...
import { fetchGamesByTag } from "../store/slices/gamesSlice"
...
const TagsPage = () => {
  ...
  const { gamesByTag: games, loading, error, totalPages } = useSelector((state) => state.games)

  useEffect(() => {
    if (tag) {
      dispatch(fetchGamesByTag({ tag, page: currentPage }))
    }
  }, [dispatch, tag, currentPage])
      ...
      <main className="flex-grow p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6 w-full max-w-6xl mx-auto">
          {games.length > 0
            ...
            : !loading && (
                <p className="text-center col-span-full text-lg">No se encontraron juegos con estos tags...</p>
              )}
        </div>
